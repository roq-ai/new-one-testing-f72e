...
import { getMenuInvitations } from 'apiSdk/menu-invitations';
...
function DashboardPage() {
  ...
  const menuInvitationStat = useSWR<PaginatedInterface<MenuInvitationInterface>>(
    () => `/menu-invitations`,
    () => getMenuInvitations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: menuInvitationStat?.data?.totalCount,
      isLoading: menuInvitationStat.isLoading,
